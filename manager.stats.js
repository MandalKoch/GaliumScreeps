 ...
 * Tracks unit counts, resource levels, RCL progression, energy gathering rates, transporter routes,
...
const managerStats = {
    ...
    /**
     * Gathers status of configured transporter routes for the room.
     * @param {Room} room
     * @returns {string}
     */
    getTransporterRouteStats: function (room) {
        const routes = managerTransporter.getRoutesForRoom(room.name);
        ...
        const routeSummaries = routes.map((route) => {
            ...
            const currentCount = transporters.filter((c) => c.memory.route === route.name).length;
            const statusIcon = currentCount >= targetCount ? '✅' : '⏳';
            return `${statusIcon} ${route.name} [${currentCount}/${targetCount}]`;
        });
    ...
    logStatistics: function (room) {
        ...
        const routeStats = this.getTransporterRouteStats(room);
        ...
        const versionStr = Object.keys(unitStats.versions)
            .map((v) => `${v}: ${unitStats.versions[v]}`)
            .join(', ');
        ...
        console.log(`👥 Units (${unitStats.total}):      ${roleStr} [${versionStr}]`);
        if (routeStats) {
            console.log(`🚚 Transporters:   ${routeStats}`);
        }
